...
import { formatPublicId, maskDbUrl, prisma } from "swyftstack-shared";
...
export default async function MigrationsPage() {
  ...
  const organizationName = imports[0]?.project.organization.name;
  return (
      ...
      <Panel title="Database imports" flush>
        <Table
          ...
          rows={imports.map((imp) => [
            ...
            <Link key="p" href={`/projects/${formatPublicId("project", imp.projectId)}`}>{imp.project.name}</Link>,
            <Badge key="s" status={imp.status} />,
            bytes(imp.sizeBytes),
            timeAgo(imp.createdAt),
            <code key="l">{maskDbUrl(imp.logs?.split("\n").at(-1) ?? "—")}</code>,
            imp.databaseId ? <Link key="d" className="btn sm secondary" href={`/projects/${formatPublicId("project", imp.projectId)}/databases/${formatPublicId("database", imp.databaseId)}`}>Open DB</Link> : "—",
          ])}
        />
      </Panel>
    </UserShell>
