...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description:
      "Managed PostgreSQL, object storage, backups and migrations on one premium developer platform.",
    url: `${SITE_URL}/`,
    type: "website",
    siteName: "Swyftstack",
  },
...
const FAQ_ITEMS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: "Yes. PostgreSQL and S3-compatible storage are open standards. You can take a pg_dump or export your buckets anytime - we'll even help.",
  },
];
...
export default async function Homepage() {
  ...
  return (
    <MarketingShell>
      ...
      {/* --- Hero ---
       * The home hero follows the theme: light bg + dark text in light mode,
       * dark bg + light text in dark mode. The Vanta mesh sits on top of
      ...
      <section className="m-hero m-hero-home">
        ...
        <div className="m-container m-hero-inner">
          ...
          <h1>
            Production-ready database and
            <span className="m-text-grad"> storage </span>
            <span className="m-text-grad">in seconds</span>.
          </h1>
          <p className="m-hero-lead">
            Deploy PostgreSQL, S3-compatible storage, and automated backups from
            one clean dashboard.
          </p>
          <div className="m-hero-ctas">
            <Link
              className="m-btn m-btn-primary m-btn-lg"
              href={signedIn ? "/console" : signupHref}
            >
              {signedIn ? "Open console" : "Deploy your first database"}{" "}
          ...
          <div className="m-hero-trust">
            <span className="m-hero-trust-item">
              <LockIcon size={14} /> SSL on by default
            </span>
            <span className="m-hero-trust-item">
              <BackupIcon size={14} /> Daily backups
            </span>
            <span className="m-hero-trust-item">
              <ClockIcon size={14} /> 47-second provisioning
            </span>
            <span className="m-hero-trust-item">
              <ShieldIcon size={14} /> 99.9% uptime SLA
            </span>
          </div>
      ...
      <Section>
        ...
        >
          <div>
            ...
            >
              No three-account scavenger hunt. Spin up a project, get a Postgres
              instance, a bucket, a backup schedule, and live usage metering -
              all addressable from one dashboard and billed on a single invoice.
            </p>
            <div className="m-grid m-grid-2 m-mt-5" style={{ gap: 14 }}>
              <Mini
                icon={<TerminalIcon size={18} />}
                title="Connection strings, ready to paste"
                body="Auto-rotated, masked in the UI, copyable everywhere."
              />
              <Mini
                icon={<LockIcon size={18} />}
                title="Scoped credentials"
                body="Per-bucket access keys. Per-database roles. No shared admin."
              />
              <Mini
                icon={<GaugeIcon size={18} />}
                title="Live metering"
                body="Storage and egress updated every minute."
              />
              <Mini
                icon={<BoltIcon size={18} />}
                title="Provisioning in seconds"
                body="No queue, no waiting room. Click, copy, ship."
              />
            </div>
          </div>
      ...
      <Section alt>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Mini
              icon={<MigrateIcon size={18} />}
              title="Read-only on your source"
              body="We pg_dump over the wire. Your existing app keeps running, untouched."
            />
            <Mini
              icon={<ShieldIcon size={18} />}
              title="Checksum-verified"
              body="Tables, row counts, indexes - verified end-to-end before we hand you the new URL."
            />
            <Mini
              icon={<ClockIcon size={18} />}
              title="Minutes, not weekends"
              body="Under 5 GB: 2-5 minutes. 5-50 GB: ~30 minutes. Bigger? We plan it with you."
            />
            <Mini
              icon={<BackupIcon size={18} />}
              title="Rollback is free"
              body="If anything looks wrong, don't switch your DATABASE_URL - your old database is still serving traffic."
            />
          </div>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Mini
              ...
              title="Prisma, Drizzle, raw pg, SQLAlchemy"
              body="No special drivers. Your ORM, your conventions."
            />
            <Mini
              icon={<TerminalIcon size={18} />}
              title="psql, pg_dump, TablePlus, DBeaver"
              body="All standard PostgreSQL tooling works unmodified."
            />
            <Mini
              icon={<BoltIcon size={18} />}
              title="Connection pooling included"
              body="PgBouncer configured for you. Toggle transaction or session mode."
            />
            <Mini
              icon={<GlobeIcon size={18} />}
              title="S3-compatible storage SDK"
              body="boto3, AWS SDK v3, Minio client - change the endpoint, leave the rest."
            />
          </div>
        </div>
      </Section>
      ...
      <Section alt>
        ...
        <div className="m-grid m-grid-3">
          <FeatureCard
            ...
            body="SSL/TLS on every connection, AES-256 at rest, encrypted backups and bucket objects."
          />
          <FeatureCard
            icon={<BackupIcon size={22} />}
            title="Tested restores"
            body="Daily backups, retention up to 30 days, and weekly restore drills. Untested backups aren't backups."
          />
          <FeatureCard
            icon={<LockIcon size={22} />}
            title="Scoped credentials"
            body="Per-bucket keys. Per-database roles. Connection IP allowlisting. SSO via SAML on Enterprise."
          />
          <FeatureCard
            icon={<GaugeIcon size={22} />}
            title="Usage limits, not bill shocks"
            body="Email alerts at 80% and 95% of any limit. Webhook alerts on Pro. No surprise overages."
          />
          <FeatureCard
            icon={<ClockIcon size={22} />}
            title="Honest status & postmortems"
            body="Live status page, automated incident detection, public postmortems within 7 days."
          />
          <FeatureCard
            icon={<TerminalIcon size={22} />}
            title="Audit logs"
            body="Every state change in the dashboard recorded with actor, IP, and target. Exportable on Pro."
          />
        </div>
      </Section>
      ...
      <Section>
        ...
        <div className="m-grid m-grid-4 m-mt-7">
          <ScaleCard
            label="Solo founders"
            body="Ship the prototype your investors keep asking about. $9/mo with the launch offer."
          />
          <ScaleCard
            label="Startups"
            body="One platform, one bill. Less to explain at your next due-diligence call."
          />
          <ScaleCard
            label="Agencies"
            body="Spin up a new project per client. Bill it through, or hand off ownership cleanly."
          />
          <ScaleCard
            label="Engineering teams"
            body="Audit logs, scoped credentials, SSO via SAML on Enterprise. Real human support."
          />
        </div>
      </Section>
      ...
      <CTASection
        ...
        subtitle="Two minutes to sign up. 47 seconds to deploy. Backed by a real humans who answer your emails."
      />
    </MarketingShell>
  );
}
...
function PriceTeaser({
  ...
  return (
    <div className={`m-plan ${popular ? "m-plan-popular" : ""}`}>
      ...
      <Link
        href="/pricing"
        className={`m-btn ${popular ? "m-btn-primary" : "m-btn-secondary"} m-btn-block`}
      >
