...
export type SshStreamCallbacks = {
...
function appendLimited(current: string, chunk: string, max = MAX_LOG_CHARS): string {
  if (current.length >= max) return current;
  const next = current + chunk;
  return next.length > max ? `${next.slice(0, max)}\n... truncated ...` : next;
}

function failedResult(error: unknown, started: number): SshResult {
...
async function tempKeyFile(encryptedKey: string): Promise<{ dir: string; file: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "swyftstack-ssh-"));
  const file = path.join(dir, "id_key");
  try {
    await fs.writeFile(file, normalizeSshPrivateKey(decryptSecret(encryptedKey)), { mode: 0o600 });
...
async function runSsh(node: Node, command: string): Promise<SshResult> {
  ...
  try {
    ...
    return failedResult(err, started);
  }
...
function processForTarget(target: NodeTarget, keyPath: string | null, command: string) {
  if (target.mode === "local") return { cmd: "sh", args: ["-lc", command] };
  if (!keyPath) throw new Error("SSH key file was not prepared.");
  return { cmd: "ssh", args: sshArgs(target, keyPath, command) };
}

async function runSshStream(
  node: Node,
  command: string,
  callbacks: SshStreamCallbacks = {},
): Promise<SshResult> {
  const target = targetForNode(node);
  const started = Date.now();
  let key: { dir: string; file: string } | null = null;
  ...
  if (target.mode === "ssh") {
    try {
      ...
      const result = failedResult(err, started);
      callbacks.onStderr?.(result.error);
      return result;
    }
  }

  const { cmd, args } = processForTarget(target, key?.file ?? null, command);
  return new Promise((resolve) => {
    ...
    const cleanup = async () => {
      if (key) {
        await fs.rm(key.dir, { recursive: true, force: true }).catch(() => undefined);
        key = null;
      }
    };
    ...
    const settle = async (exitCode: number | null) => {
      ...
      if (timedOut) error = appendLimited(error, "\nCommand timed out.");
      if (aborted) error = appendLimited(error, "\nCommand aborted by client.");
      await cleanup();
      resolve({
    ...
    const timeout = setTimeout(() => {
      ...
      child.kill("SIGTERM");
      setTimeout(() => child.kill("SIGKILL"), 3000).unref();
    }, callbacks.timeoutMs ?? COMMAND_TIMEOUT_MS);
    timeout.unref();
    ...
    child.stderr.on("data", (chunk: Buffer) => {
      const text = chunk.toString();
      error = appendLimited(error, text);
      callbacks.onStderr?.(text);
    });
    child.on("error", async (err) => {
      error = appendLimited(error, err.message);
...
export const sshNodeService = {
  ...
  async runCommand(nodeId: string, command: string) {
    const node = await prisma.node.findUniqueOrThrow({ where: { id: nodeId } });
    const result = await runSsh(node, command);
    await recordLog(node.id, "run_command", command, result, { updateStatus: false });
    return result;
  },

  async runCommandStream(nodeId: string, command: string, callbacks: SshStreamCallbacks = {}) {
    const node = await prisma.node.findUniqueOrThrow({ where: { id: nodeId } });
    const result = await runSshStream(node, command, callbacks);
    await recordLog(node.id, "run_command", command, result, { updateStatus: false });
    return result;
  },
};
