...
import { prisma, type ProviderHelpBody } from "swyftstack-shared";
import { Icon } from "@/components/icons";
import { Panel, KeyValue } from "@/components/ui";
import { Tabs } from "@/components/client";

export const dynamic = "force-dynamic";
export const metadata = { title: "Help & Guides — Swyftstack" };
    ...
    <>
      <Panel title="How node connectivity works">
        <p className="small" style={{ margin: "0 0 10px" }}>
          Swyftstack connects to every VPS over standard SSH. You paste the node&apos;s <strong>private key</strong> once
          when registering it — Swyftstack encrypts it at rest with AES-256-GCM and uses it for the connection test,
      ...
      <Panel title="Step 1 — Generate an SSH key for Swyftstack">
        <Code>{`ssh-keygen -t ed25519 -C "swyftstack-node" -f ~/.ssh/swyftstack_node

# Public key  -> add to the VPS provider
cat ~/.ssh/swyftstack_node.pub

# Private key -> paste into Swyftstack (Nodes -> Add node)
cat ~/.ssh/swyftstack_node`}</Code>
      </Panel>
