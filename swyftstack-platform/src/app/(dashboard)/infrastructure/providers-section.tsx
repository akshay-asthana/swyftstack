// Infrastructure → Database Clusters / Object Storage / Backup Storage / Worker
// Configs. DB-managed customer infrastructure; credentials encrypted at rest.
  ...
  return (
    <>
      <p className="small" style={{ marginBottom: 12 }}>
        Add a cluster → test the connection → it becomes available for
        provisioning. New databases are placed via the <strong>database</strong>{" "}
        provisioning policy.
      </p>
      <Table
...
export async function ProvidersSection({ tab }: { tab: string }) {
  if (tab === "object-storage") return <ObjectStorage />;
  if (tab === "backup-storage") return <BackupStorage />;
