...
export async function GET() {
  const script = `#!/bin/sh
set -e
echo "Quickdock node-agent installer (placeholder)"
...
# docker run -d --name qd-agent -e NODE_TOKEN="$NODE_TOKEN" -e CONTROL_PLANE="$CONTROL_PLANE" quickdock/node-agent:latest
