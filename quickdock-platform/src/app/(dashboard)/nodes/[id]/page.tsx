...
async function runCommand(formData: FormData) {
  ...
  if (command && command.length <= 1000) await sshNodeService.runCommand(id, command).catch(() => undefined);
  revalidatePath(`/nodes/${id}`);
}
  ...
  const logsTab = (
    <>
      <Panel title="Run a command">
        <form action={runCommand}>
          <input type="hidden" name="id" value={node.id} />
          <input name="command" placeholder="docker ps --format '{{.Names}}: {{.Status}}'" maxLength={1000} />
          <div style={{ marginTop: 10 }}><button className="btn" type="submit">Run command</button></div>
        </form>
        {latestLog && (
          <pre className="log" style={{ marginTop: 14 }}>
            {latestLog.output || latestLog.error || "No output."}
          </pre>
        )}
      </Panel>
      <Panel title="Connection log" flush>
        <Table
          columns={["Time", "Action", "Status", "Exit", "Duration"]}
